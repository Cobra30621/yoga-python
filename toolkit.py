...
def treePoseRule(roi, tips, sample_angle_dict, angle_dict, point3d):
    ...
    for key, _ in roi.items():
        ...
        if key == 'RIGHT_KNEE':
            ...
            index_x,_,_,index_vi = point3d[AngleNodeDef.LEFT_INDEX] if key == 'LEFT_INDEX' else point3d[
                AngleNodeDef.RIGHT_INDEX]
...
def plankPoseRule(roi, tips, sample_angle_dict, angle_dict, point3d):
    ...
    for key, value in roi.items():
        ...
        if tips == "":
            tip_flag = True
        if key == 'NOSE':
            if point3d[AngleNodeDef.NOSE][0] > point3d[AngleNodeDef.LEFT_HIP][0] and point3d[AngleNodeDef.NOSE][0] > point3d[
                AngleNodeDef.RIGHT_HIP][0]:
                roi['NOSE'] = True
                side = 'RIGHT_'
            elif tip_flag == True:
                roi['NOSE'] = True
        ...
        if key == side + 'EYE':
            if side == 'LEFT_':
                if point3d[AngleNodeDef.RIGHT_SHOULDER][3] <MIN_DETECT_VISIBILITY and point3d[AngleNodeDef.RIGHT_EYE][3] < MIN_DETECT_VISIBILITY:
                    continue
                eye_shoulder_distance = abs(point3d[AngleNodeDef.RIGHT_SHOULDER][1] - point3d[
                    AngleNodeDef.RIGHT_EYE][1])
